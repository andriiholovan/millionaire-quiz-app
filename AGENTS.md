...
| :--- | :--- | :--- |
| `@/*` | `./*` | Root fallback |
| `@actions` | `./app/_actions` | Next.js Server Actions |
| `@components` | `./app/_components` | Reusable React UI components |
...
│   │   ├── assets/        # SVG icons as React components (e.g. HandIcon)
...
- Ensure imports use path aliases (`@components`, `@lib/server`, etc.) instead of long relative paths.
